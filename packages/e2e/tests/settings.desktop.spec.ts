...
test.describe("Settings", (): void => {
  test.describe("General", (): void => {
    ...
    test.skip("should toggle 'Allow signups'", async ({ page }) => {
      ...
      const newToggleButton = page.locator(
        "button[aria-label='Allow signups']",
      );
      const new_toggle_value =
        await newToggleButton.getAttribute("aria-checked");
      expect(Boolean(initial_toggle_value)).toBe(new_toggle_value);
    });
    ...
    test.skip("should toggle 'Developer mode'", async ({ page }) => {
      ...
      const new_toggle_value =
        await newToggleButton.getAttribute("aria-checked");
      expect(initial_toggle_value).toBe(new_toggle_value);
    });
