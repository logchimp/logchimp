...
export const siteSetup = async ({
  ...
  return await axios({
    method: "POST",
    url: `${import.meta.env.VITE_API_URL}/api/v1/auth/setup`,
    data: {
      siteTitle,
      name,
      email,
...
export const uploadSiteLogo = async (logo: FormData) => {
  ...
  return await axios({
    method: "POST",
    url: `${import.meta.env.VITE_API_URL}/api/v1/settings/update-logo`,
    data: logo,
    headers: {
      Authorization: `Bearer ${authToken}`,
...
export const getLabsSettings = async () => {
  return await axios({
    method: "GET",
    url: `${import.meta.env.VITE_API_URL}/api/v1/settings/labs`,
  });
};
