...
describe("create post", () => {
  ...
  describe('have \'post:create\' permission', () => {
    ...
    it('should not have CTA button disabled', () => {
      expect(wrapper.find('[data-test="create-post-button"]').classes('button-primary-disabled')).toBe(false)
    })
    ...
    it("show error message on empty title", async () => {
      ...
      expect(
        wrapper
          .find("[data-test=post-title] [data-test=input-error-message]").exists()
      ).toBeTruthy()
    });
  })
